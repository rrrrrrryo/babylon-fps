...
import { MarbleProceduralTexture, BrickProceduralTexture } from "@babylonjs/procedural-textures";
import * as cannon from "cannon";

export const createScene = async function(engine, canvas){
    const size = 48;
    const maze = new Maze(6, 8);
    const heigth = maze.h * size;
    const width = maze.w * size;
    const name = "test";
    // scene
    const scene = new BABYLON.Scene(engine);
    scene.enablePhysics(new BABYLON.Vector3(0, -9.8, 0), new BABYLON.CannonJSPlugin(true, 100, cannon));

    // player
    const player = new Player(scene, new BABYLON.Vector3(size*1.5 - width/2, 6, size*1.5 - heigth/2))
    ...
    // // Mesh
    const roadmaterialpt = new MarbleProceduralTexture("marble", 512, scene);
    roadmaterialpt.uScale = maze.w;
    roadmaterialpt.vScale = maze.h;
    const ground =  BABYLON.MeshBuilder.CreatePlane('ground', {height:heigth, width:width}, scene);
    // const ground = BABYLON.MeshBuilder.CreateBox("ground", {heigth, width}, scene);
    ground.rotation.x = Math.PI / 2;
    ground.materal = new BABYLON.StandardMaterial("road", scene);
    ground.materal.diffuseTexture = roadmaterialpt;
    ...
    brickMaterial.diffuseTexture = new BrickProceduralTexture(name + "text", 512, scene);
    const boxes = maze.units
        ...
        .map((e, index) => {
            ...
            box.position = new BABYLON.Vector3((e.j + 0.5) * size - width / 2, size / 2, (e.i + 0.5) * size - heigth / 2)
            return box
        })
    ...
    scene.registerBeforeRender(e => {
        // 速度
        const force = 3 * engine.getDeltaTime()
        if (player.isOnGround(scene)) {
            let velocity = new BABYLON.Vector3(0,0,0)
            if (keys[32]) {
                player.body.physicsImpostor.applyImpulse(new BABYLON.Vector3(0, 200, 0), player.body.getAbsolutePosition())
            } else {
                if (keys[87]) {
...
class Player {
    ...
    getBack() {
        const func = this.camera.globalPosition.subtract(this.head.absolutePosition);
        func.y = 0;
...
class Unit {
    ...
    updateNA(data){
        const {i, j} = this
        if (data[i][j-2]){
            this.neighborArry.push(data[i][j-2]);
            data[i][j-2].neighborArry.push(this);
        }
        if (data[i-2]){
            this.neighborArry.push(data[i-2][j]);
            data[i-2][j].neighborArry.push(this)
        }
    }
}
...
class Maze{
    constructor(row, col){
        ...
        while (visiteds.length > 0){
            ...
            neighbors = current.neighborArry.filter(e => !e.isVisited)
